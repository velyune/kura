...
fn encode_count_u32(count: usize, message: &str) -> Result<u32> {
    u32::try_from(count).map_err(|_| Error::EncodingLimitExceeded {
...
fn write_snapshot(file: &mut File, state: &ManifestState) -> Result<()> {
    ...
    let wal_count = encode_count_u32(
        state.wal_files().len(),
        "manifest snapshot WAL file count exceeds u32",
    )?;
    ...
    let sstable_count = encode_count_u32(
        state.sstables().len(),
        "manifest snapshot SSTable count exceeds u32",
    )?;
...
fn write_sstable_descriptor(file: &mut File, sstable: &SstableDescriptor) -> Result<()> {
    ...
    let min_user_key_len = encode_count_u32(
        sstable.min_user_key().len(),
        "manifest SSTable descriptor min user key length exceeds u32",
    )?;
    file.write_all(&min_user_key_len.to_le_bytes())?;
    file.write_all(sstable.min_user_key())?;

    let max_user_key_len = encode_count_u32(
        sstable.max_user_key().len(),
        "manifest SSTable descriptor max user key length exceeds u32",
    )?;
...
mod tests {
    ...
    const INITIAL_MANIFEST_BYTES: &[u8] = &[
        b'K', b'U', b'R', b'A', b'M', b'N', b'F', // magic bytes
        1, 0, // format version: u16
        2, 0, 0, 0, 0, 0, 0, 0, //  next file number: u64
        0, 0, 0, 0, 0, 0, 0, 0, // last sequence: u64
        0, 0, 0, 0, // WAL file count: u32
        0, 0, 0, 0, // SSTable count: u32
    ];
    ...
    #[test]
    fn load_reads_initial_manifest_file() {
        let (_temp, db_path) = temp_db();
        let manifest_path = db_path.join(filename::manifest(manifest::INITIAL_FILE_NUMBER));

        fs::write(&manifest_path, INITIAL_MANIFEST_BYTES).expect("write initial manifest bytes");
        let state = load(&manifest_path).expect("load initial manifest");

        assert_eq!(state, ManifestState::initial());
    }
    ...
    fn load_rejects_truncated_manifest_header() {
        ...
        assert!(matches!(err, Error::Corruption {message}
            if message == format!(
                "manifest header is truncated while reading magic: {}",
                manifest_path.display()
            )
        ));
    }
    ...
    fn load_rejects_unsupported_manifest_format_version() {
        ...
        assert!(matches!(err, Error::Corruption {message}
            if message == format!(
                ...
                MANIFEST_FORMAT_VERSION + 1, manifest_path.display()
            )
        ));
    }
    ...
    fn load_rejects_truncated_manifest_snapshot() {
        ...
        assert!(matches!(err, Error::Corruption {message}
            if message == format!(
                "manifest snapshot is truncated while reading next file number: {}",
                manifest_path.display()
            )
        ));
    }
    ...
    #[test]
    fn write_creates_initial_manifest_file() {
        let (_temp, db_path) = temp_db();
        let manifest_path = db_path.join(filename::manifest(manifest::INITIAL_FILE_NUMBER));

        write(&manifest_path, &ManifestState::initial()).expect("write initial manifest state");
        let bytes = fs::read(&manifest_path).expect("read initial manifest bytes");

        assert_eq!(bytes, INITIAL_MANIFEST_BYTES);
    }

    #[test]
    #[cfg(target_pointer_width = "64")]
    fn encode_count_u32_rejects_count_overflow() {
        let err = encode_count_u32(u32::MAX as usize + 1, "count exceeds u32")
            .expect_err("encode_count_u32 should reject count overflow");

        assert!(matches!(err, Error::EncodingLimitExceeded {message}
            if message == "count exceeds u32"
        ));
    }
}
