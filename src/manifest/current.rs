...
mod tests {
    ...
    #[test]
    fn parse_rejects_current_with_invalid_manifest_filename() {
        let current = b"MANIFEST-1\n";

        let err = parse(current).expect_err("parse should reject invalid manifest filename");
    ...
    fn publish_replaces_existing_current() {
        ...
        let new_manifest_filename = filename::manifest(manifest::INITIAL_FILE_NUMBER + 1);
    ...
    #[test]
    fn publish_rejects_invalid_manifest_filename() {
        let (_temp, db_path) = temp_db();

        let err = publish(&db_path, "MANIFEST-1")
            .expect_err("publish should reject invalid manifest filename");
        ...
        assert!(
            ...
            "CURRENT should not be created for invalid manifest filename"
        );
    }
}
